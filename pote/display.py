...
# %% auto #0
__all__ = ['ESC', 'sgr', 'color_table', 'color_codes', 'dir_tree', 'show_tree', 'print_tree', 'FileMeta', 'add_style', 'list2lis',
           'dict2uls', 'dict2dtls', 'DetailsColl', 'DetailsJSON']
...
# %% ../nbs/01_display.ipynb #2bdbbc81
def dir_tree(path, exclude=None, on_file=None, on_dir=None):
    path = Path(path)
    exclude = set(exclude or [])
    def _collect(p):
        for o in sorted(p.iterdir(), key=lambda x: x.name.lower()):
            if o.name in exclude: continue
            if o.is_dir():
                children = list(_collect(o))
                yield on_dir(o, children) if on_dir else {o: children}
            else:
                yield on_file(o) if on_file else o
    return _collect(path)

# %% ../nbs/01_display.ipynb #19ee67ea
def show_tree(path, exclude=None):
    def _file(o): return Li(o.name)
    def _dir(o, ch): return Li(Details(Summary(o.name), Ul(*ch)))
    show(Ul(*dir_tree(path, exclude, on_file=_file, on_dir=_dir)))

# %% ../nbs/01_display.ipynb #c42ad5fa
def print_tree(path, exclude=None, color=True):
    tr = dir_tree(path, exclude, on_file=lambda o: o.name)
    _d,_r,_g = ('\033[34m','\033[0m','\033[38;5;7m') if color else ('','','')
    def _print(items, prefix=''):
        items = list(items)
        for i,o in enumerate(items):
            last = i == len(items)-1
            conn,ext = ('└─ ','   ') if last else ('├─ ','│  ')
            if isinstance(o, str): print(_g + prefix + conn + _r + o)
            else:
                dir,children = next(iter(o.items()))
                print(_g + prefix + conn + _r + _d + dir.name + _r)
                _print(children, prefix+ext)
    _print(tr)

# %% ../nbs/01_display.ipynb #367ea335
class FileMeta(Path):
...
# %% ../nbs/01_display.ipynb #e079b695
def it2lbl2(it, nm=''):
    return (it,) if isinstance(it, FT) else (str(it),)

# %% ../nbs/01_display.ipynb #3f3b7bf2
